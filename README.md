...
2. 按照下面的选项和参数表构造运行命令。
   ...
   一般格式：`bash NJUPT-AutoLogin.sh [-i interface] [-I isp] [-t timeout] [-p ipv4_addr] [-r] [-T sleep_time] [-6] [-m] [-n] [-c] [-l] [-h] [-v] login_id login_password`
3. 在终端内运行命令。
...
```crontab
*/5 * * * * bash /path/to/your/NJUPT-AutoLogin.sh [-i interface] [-I isp] [-t timeout] [-p ipv4_addr] [-r] [-T sleep_time] [-6] [-m] [-n] [-c] [-l] [-v] login_id login_password
```
